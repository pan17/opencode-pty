export const SPAWN_DESCRIPTION = `Spawns a new interactive PTY (pseudo-terminal) session that runs in the background.
...
- The \`command\` parameter MUST include the file extension on Windows. The underlying
  PTY binding (@lydell/node-pty) calls CreateProcessW with the literal command name and
  does not perform PATHEXT extension resolution, so "npm" or "node" will fail with
  "Cannot create process, error code: 2" — even when those commands are on PATH.
- Common extensions to add explicitly: \`npm.cmd\`, \`npx.cmd\`, \`pnpm.cmd\`,
  \`yarn.cmd\`, \`tsc.cmd\`, \`node.exe\`, \`ping.exe\`. The CLI tools installed by
  Node are typically \`.cmd\` shims; native binaries are typically \`.exe\`.
- As a portable fallback for shell metacharacters (pipes, redirects, \`&&\`),
  wrap the command in \`cmd.exe\`: \`command="cmd.exe", args=["/c", "ping", "127.0.0.1"]\`.
- Use an absolute path (e.g., \`C:\\\\Windows\\\\System32\\\\ping.exe\`) to bypass PATH
  lookup entirely if you know the install location.
`
